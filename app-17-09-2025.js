...
// === Ajustes rápidos ===
const DAYS_BACK = 7; // Ventana de búsqueda
//const ZONES_ARRAY = ['1. Living', '2. Cocina', '3. Playroom'];
const ZONES_ARRAY = ['Habitación de los niños'];
...
module.exports = class MyApp extends Homey.App {
  ...
  try {
    ...
    this.lightChangedTriggerCard = this.homey.flow.getTriggerCard('light_changed');
  ...
  async getHistoryByZoneReal(lightsInZone) {
    ...
    await Promise.all(lightsInZone.map(async light => {
      ...
      for (const e of entries) {
        ...
        // Formatear los datos con todos los campos requeridos
        const logEntry = {
          event_date: this.fmtYMD.format(d), // Fecha en formato YYYY-MM-DD
          day_of_week: d.toLocaleDateString('es-ES', { weekday: 'long' }), // Día de la semana
          month_name: d.toLocaleDateString('es-ES', { month: 'long' }), // Mes
          event_time: this.fmtHM.format(d), // Hora en formato HH:mm
          value_bool: v, // Valor booleano
          duration_in_state_seconds: e.durationInState || null, // Duración en estado (si está disponible)
          zone: light.zone,
          deviceId: light.id,
          name: light.name
        };
...
async guardarDatosDeLaSemana(logEntry) {
  ...
  // Puedes agregar validaciones extra si lo deseas
  try {
    ...
    try {
      ...
      throw new Error(`La respuesta no es JSON válido: ${rawResponse}`);
    }

    if (result.error !== "0") {
      console.error("❌ Error guardando log:", result);
    }
  } catch (error) {
    console.error("❌ Error en la conexión con la API de Pantea Smart:", error);
  }
}
